...
// side; apps/web/src/sections/Lab.defaults.test.ts pins this one. Both are a stopgap
...
/**
 * What the backend will really use for track association, and whether the control the
 * operator is turning does anything at all.
 *
 * webui/lab_core.py:518 computes `max(roi_dedup_px, 50 * (1 - match_thresh))`, so with
 * the shipped 25 px and 0.70 the second term is 15 and is discarded — the whole upper
 * half of the slider produces identical runs. Rather than quietly leave that hidden,
 * the form shows the derived value and says when the knob is inert.
 */
...
function confidenceBinLabel(bin: DetectionConfidenceBin): string {
...
export function Lab() {
  ...
  function downloadEventsCsv() {
    ...
    const columns = ["event_id", "sequence", "frame_index", "timestamp_ms", "track_id", "class_id", "centroid_x", "centroid_y", "bbox_x1", "bbox_y1", "bbox_x2", "bbox_y2", "direction", "side_before", "side_after", "status", "recovery", "detection_conf", "exclusion_zone_id", "raw_conf", "exclusion_hit", "reason", "path_points", "path_predictor", "path_total_displacement"] as const;
    ...
    const rows = result.events.map((event) => [
      ...
      event.exclusion_zone_id, event.provenance?.decision?.raw_conf,
      ...
      event.provenance?.path?.predictor ? predictorLabel(event.provenance.path.predictor) : undefined, event.provenance?.path?.total_displacement,
  ...
  const capabilityReason = (name: keyof LabCapabilities) => {
    ...
    if (backendUnsupportedCapabilities.includes(name)) return `Backend reports ${name.split("_").join(" ")} is unsupported.`;
  ...
  return <div className="lab-shell">
    ...
    <div className="lab-workspace">
      <aside className="lab-sidebar">{sections.map((section) => <section className="lab-accordion" key={section.n}><SectionHeader section={section} onToggle={() => toggleSection(section.n)} />{section.open && <div className="lab-section-body">
        ...
        {section.n === 3 && <><span className="lab-capability-state">{capability("tracker") ? "TRACKER WIRED" : "TRACKER CAPABILITY REQUIRED"}</span><DisabledField><span>Tracker <Hint text="What this backend actually runs: a deterministic greedy nearest-centroid tracker (webui/lab_core.py). NOT ByteTrack — the device runs ByteTrack, and until the shared counting engine lands these two count differently. This control used to say ByteTrack, which was untrue." /></span><select disabled={!capability("tracker")} value={cfg.tracker_type ?? "centroid"} onChange={(e) => set("tracker_type", e.target.value)}><option value="centroid">Centroid — greedy nearest-centroid</option></select></DisabledField><DisabledField><span>Track buffer <Hint text="Frames an unmatched track remains alive before removal." /></span><input disabled={!capability("tracker")} type="number" min="1" max="300" step="1" value={cfg.track_buffer ?? 30} onChange={(e) => set("track_buffer", Math.max(1, Math.min(300, Math.floor(Number(e.target.value) || 1))))} /></DisabledField><DisabledField><span>Match threshold <Hint text="Folded into a pixel match distance as max(roi_dedup_px, 50*(1-match_thresh)) in webui/lab_core.py. It is NOT ByteTrack's IoU association cost despite the borrowed name, and above the crossover it has no effect at all — the ROI dedup radius wins." /></span><input disabled={!capability("tracker")} type="number" min="0" max="1" step="0.01" value={cfg.match_thresh ?? 0.7} onChange={(e) => set("match_thresh", Math.max(0, Math.min(1, Number(e.target.value) || 0)))} />{(() => { const d = matchDistancePx(cfg); return <span className={d.inert ? "lab-derived-inert" : "lab-derived"}>effective match distance {d.px} px{d.inert ? ` — this control has no effect at ${cfg.match_thresh ?? 0.7}; the ROI dedup radius (${cfg.roi_dedup_px ?? 25} px) wins below ${d.crossover.toFixed(2)}` : ""}</span>; })()}</DisabledField></>}
      ...
      <aside className="lab-inspector"><section className="lab-inspector-card"><div className="lab-card-title"><span>OUTPUT PREVIEW</span>{result && <span className="lab-ok">READY</span>}</div>{result ? <><video src={result.video_url} controls /><div className="lab-stat-grid">{stats.map((stat) => <div key={stat.label}><strong>{stat.value}</strong><span>{stat.label}</span></div>)}</div><a className="lab-download-link" href={result.video_url} download><Download size={14} /> Download overlay.mp4</a></> : <div className="lab-empty-inspector"><Layers3 size={22} /><span>Run Replay to populate the backend output.</span></div>}</section>
        ...
        <section className="lab-inspector-card"><div className="lab-card-title"><span>RUN HISTORY</span><span className={`lab-chip ${runHistory?.length ? "lab-chip-live" : ""}`}>{runHistoryLoading ? "LOADING" : runHistory?.length ? `${runHistory.length} ${runHistoryPersistent === false ? "THIS SESSION" : "SAVED"}` : "EMPTY"}</span></div>{runHistoryPersistent === false && !runHistoryLoading && <div className="lab-capability-notice">Run history lives in the backend process, not on disk — restarting it loses every manifest here, and a saved task will then list run ids with nothing behind them.</div>}{runHistoryLoading ? <div className="lab-research-empty">Checking the backend run store…</div> : runHistoryError ? <div className="lab-research-empty lab-history-locked">Run history is unavailable. No local history is shown.</div> : runHistory?.length ? <div className="lab-recent-runs">{runHistory.slice(0, 5).map((run) => <div className="lab-recent-run" key={run.run_id}><code>{run.run_id}</code><span>{run.schema_version}</span><small>{run.created_at}</small></div>)}</div> : <div className="lab-research-empty">The backend returned no saved runs yet.</div>}</section>
        <section className="lab-inspector-card lab-compare-card"><div className="lab-card-title"><span>A/B EVENT + CONFIG COMPARE</span><span className={`lab-chip ${runCompare ? "lab-chip-live" : ""}`}>{runHistoryLoading ? "LOADING" : runHistoryError ? "LOCKED" : runHistory && runHistory.length >= 2 ? "READY" : "2 RUNS REQUIRED"}</span></div>{runHistoryLoading ? <div className="lab-research-empty">Waiting for backend RunManifest history…</div> : runHistoryError ? <div className="lab-research-empty lab-history-locked">Compare is locked because backend run history is unavailable. No local runs are substituted.</div> : runHistory && runHistory.length >= 2 ? <><div className="lab-compare-selects"><label>Baseline<select value={baselineRunId} onChange={(event) => setBaselineRunId(event.target.value)}>{runHistory.map((run) => <option key={run.run_id} value={run.run_id}>{run.run_id}</option>)}</select></label><label>Current<select value={currentRunId} onChange={(event) => setCurrentRunId(event.target.value)}>{runHistory.map((run) => <option key={run.run_id} value={run.run_id}>{run.run_id}</option>)}</select></label></div>{runCompare ? <><div className="lab-compare-ids"><span>baseline <code>{runCompare.baselineId}</code></span><span>current <code>{runCompare.currentId}</code></span></div><div className="lab-compare-block lab-event-diff-block"><span className="lab-output-label">EVENT-LEVEL DIFF · PRIMARY</span>{runCompare.eventDiff.locked ? <div className="lab-locked-output"><strong>EVENT DIFF LOCKED</strong><span>Both manifests must return backend-owned counts.events.event_records[]. No event identity is inferred from aggregate counts or event_ids.</span></div> : <><div className="lab-event-diff-summary"><strong>{runCompare.eventDiff.addedIds.length}</strong><span>added</span><strong>{runCompare.eventDiff.removedIds.length}</strong><span>removed</span><strong>{runCompare.eventDiff.changedRecords.length}</strong><span>changed</span></div>{(runCompare.eventDiff.addedIds.length > 0 || runCompare.eventDiff.removedIds.length > 0) && <div className="lab-event-id-columns">{runCompare.eventDiff.addedIds.length > 0 && <div><span className="lab-output-label">ADDED IDS</span><div className="lab-key-list">{runCompare.eventDiff.addedIds.map((id) => <code key={id}>{id}</code>)}</div></div>}{runCompare.eventDiff.removedIds.length > 0 && <div><span className="lab-output-label">REMOVED IDS</span><div className="lab-key-list"><code className="lab-removed-id">{runCompare.eventDiff.removedIds.map((id) => id).join(" ")}</code></div></div>}</div>}{runCompare.eventDiff.changedRecords.length > 0 && <div className="lab-changed-events">{runCompare.eventDiff.changedRecords.map((change) => <article className="lab-changed-event" key={change.eventId}><div className="lab-changed-event-head"><code>{change.eventId}</code><span>{change.changedFields.join(" · ")}</span></div><div className="lab-changed-event-values"><div><span>baseline</span><code>frame {diagnosticObjectValue(change.baseline.frame_index)} · {diagnosticObjectValue(change.baseline.direction)} · {diagnosticObjectValue(change.baseline.status)} · conf {safeNumber(change.baseline.detection_conf)}</code></div><div><span>current</span><code>frame {diagnosticObjectValue(change.current.frame_index)} · {diagnosticObjectValue(change.current.direction)} · {diagnosticObjectValue(change.current.status)} · conf {safeNumber(change.current.detection_conf)}</code></div></div>{change.changedFields.some((field) => field.startsWith("decision.")) && <div className="lab-changed-decision"><span>decision provenance</span><code>{diagnosticObjectValue(change.baseline.decision)} → {diagnosticObjectValue(change.current.decision)}</code></div>}</article>)}</div>}{runCompare.eventDiff.addedIds.length === 0 && runCompare.eventDiff.removedIds.length === 0 && runCompare.eventDiff.changedRecords.length === 0 && <p className="lab-note">No event-level differences in the backend-owned records.</p>}</>}</div><div className="lab-compare-block lab-aggregate-secondary"><span className="lab-output-label">AGGREGATE METRIC DELTAS · SECONDARY</span>{runCompare.metricDeltas.length ? <div className="lab-delta-grid">{runCompare.metricDeltas.map((metric) => <div key={metric.key}><strong className={metric.delta > 0 ? "lab-delta-positive" : metric.delta < 0 ? "lab-delta-negative" : ""}>{metric.delta > 0 ? "+" : ""}{metric.delta}</strong><span>{metric.key} · {metric.baseline} → {metric.current}</span></div>)}</div> : <p className="lab-note">No shared numeric metrics were returned by both manifests. Nothing is inferred.</p>}</div><div className="lab-compare-block"><span className="lab-output-label">CHANGED CONFIG KEYS</span>{runCompare.changedConfigKeys.length ? <div className="lab-key-list">{runCompare.changedConfigKeys.map((key) => <code key={key}>{key}</code>)}</div> : <p className="lab-note">No changed keys in the backend snapshots.</p>}</div><div className="lab-replay-controls"><span className="lab-output-label">REPLAY CONFIG</span><p className="lab-note">Load a selected manifest’s safe config into the editor. This changes local fields only; it never runs replay or changes the model.</p><div className="lab-compare-actions"><button className="lab-button lab-button-cyan" type="button" onClick={() => baselineRun && loadManifestConfig(baselineRun)}>Load baseline config</button><button className="lab-button lab-button-cyan" type="button" onClick={() => currentRun && loadManifestConfig(currentRun)}>Load current config</button></div></div></> : <div className="lab-research-empty">Choose two different backend manifests to compare.</div>}</> : <div className="lab-research-empty lab-history-locked">A/B compare is locked until the backend returns at least two actual RunManifests. No counts, events, or deltas are estimated.</div>}</section>
        <section className="lab-inspector-card"><div className="lab-card-title"><span>COUNT SUMMARY</span><span className={`lab-chip ${hasSummary ? "lab-chip-live" : ""}`}>{hasSummary ? "BACKEND SUMMARY" : "SUMMARY REQUIRED"}</span></div>{summary ? <div className="lab-summary-grid">{(["confirmed", "flagged", "dropped", "recovered", "excluded", "total"] as const).map((key) => <div className={`lab-summary-value lab-summary-${key}`} key={key}><strong>{summary[key]}</strong><span>{key.replace("_", " ")}</span></div>)}{summary.ground_truth != null && <div className="lab-summary-value lab-summary-gt"><strong>{summary.ground_truth}</strong><span>GT target</span></div>}{summary.error_vs_ground_truth != null && <div className="lab-summary-value lab-summary-error"><strong>{summary.error_vs_ground_truth}</strong><span>error vs GT</span></div>}{(summary.tolerance_pct != null || summary.tolerance_state != null) && <div className={`lab-summary-note lab-tolerance-${summary.tolerance_state ?? "unknown"}`}><span>tolerance</span><strong>{summary.tolerance_pct != null ? `${(summary.tolerance_pct * 100).toFixed(1)}%` : "—"}</strong>{summary.tolerance_state && <em>{summary.tolerance_state.toUpperCase()}</em>}</div>}</div> : <div className="lab-research-empty">{result ? "Backend returned no result.summary. Counts are not estimated from detection frames." : "Run Replay to show a backend-provided count summary."}</div>}</section>
        <section className="lab-inspector-card lab-diagnostics-card"><div className="lab-card-title"><span>DETECTION DIAGNOSTICS</span><span className={`lab-chip ${hasDiagnostics ? "lab-chip-live" : ""}`}>{hasDiagnostics ? "BACKEND OUTPUT" : "LOCKED"}</span></div><p className="lab-diagnostics-disclaimer">Detector diagnostics only — these are not accuracy or ground-truth metrics.</p>{diagnostics ? <div className="lab-diagnostics-content"><div className="lab-diagnostics-metrics"><div><strong>{diagnosticObjectValue(diagnostics.total_detections)}</strong><span>total detections</span></div><div><strong>{diagnosticObjectValue(diagnostics.frames_with_detections)}</strong><span>frames with detections</span></div><div><strong>{diagnostics.sampled_frame_density.length}</strong><span>sampled density frames</span></div></div><div className="lab-diagnostics-block"><span className="lab-output-label">DETECTIONS BY CLASS</span>{Object.entries(diagnostics.detections_by_class ?? {}).length ? <div className="lab-diagnostics-class-list">{Object.entries(diagnostics.detections_by_class).map(([classId, count]) => <div key={classId}><span>class {diagnosticObjectValue(classId)}</span><strong>{diagnosticObjectValue(count)}</strong></div>)}</div> : <p className="lab-note">Backend returned no per-class counts.</p>}</div><div className="lab-diagnostics-block"><span className="lab-output-label">CONFIDENCE HISTOGRAM</span>{diagnostics.confidence_histogram?.bins?.length ? <div className="lab-diagnostics-histogram">{diagnostics.confidence_histogram.bins.map((bin, index) => <div key={`${confidenceBinLabel(bin)}-${index}`}><span>{confidenceBinLabel(bin)}</span><strong>{diagnosticObjectValue(bin.count)}</strong></div>)}</div> : <p className="lab-note">Backend returned no histogram bins.</p>}</div></div> : <div className="lab-locked-output"><strong>DETECTOR DIAGNOSTICS LOCKED</strong><span>{result ? "This result did not include backend diagnostics." : "Run Replay to show backend-provided detector diagnostics."}</span><span>No detection metrics are estimated in the browser.</span></div>}</section>
