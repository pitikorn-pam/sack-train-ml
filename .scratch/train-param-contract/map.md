...
- [Prototype the New-run form under the new contract](./issues/09-prototype-new-run-form.md) — **v3 accepted**: an externally-reviewed form (family/task/size decomposition, dataset card with real counts, presets, run name, Summary/YAML tabs) with two contract pieces ported back — a level that *blocks* known-broken paths instead of cautioning, and provenance on every value, so the config reads `optimizer: AdamW  # default — nobody typed this`. Catalogue verified against the official docs.
- [Does DFC auto-insert the cls sigmoid?](./issues/11-verify-cls-sigmoid-in-alls.md) — **it does**, on all three cls convs, triggered by `nms_postprocess(meta_arch=yolov8)` and observed directly on the deployed `f8f85a5d`. Our ALLS needs no `change_output_activation`, and the worst available explanation for the INT8 collapse — every HEF ever built here feeding raw logits to a probability threshold — is eliminated. The collapse goes back to `optimization_level`. Unplanned second finding: synthetic calibration frames made DFC *refuse to quantize*, which real frames fixed.
- [Which parameters are first-class fields, and what does the escape hatch allow?](./issues/06-fields-vs-escape-hatch.md) — every parameter lands in one of **four** categories: field, advanced escape hatch, **derived (shown, never editable)**, or refused at submit. `imgsz` stays editable and the compile *inherits* it. Both forms' fields are fixed, each with a default and help text; `deterministic` and `save_period` are added; the catalogue lists every trainable checkpoint with its capability marked, which pulls the head-index fix into scope so those labels are true.
- [Separating compile from train: what happens to `compile_run.ipynb`?](./issues/08-compile-notebook-and-recompile-flow.md) — **a compile is a run of its own kind**, carrying `source_run_id`; both kinds share **one launcher notebook** that dispatches on kind, which dissolves the stale `compile_run.ipynb` copy; configuration comes from the registry, with an override path that is allowed but cannot publish artifacts; and **calibration sets become named R2 artifacts referenced by key**, fixing the unreproducible disk path in `dc3dbb2`.
- [Which layer refuses a bad config?](./issues/04-where-validation-lives.md) — **static** validation (keys, ranges, cross-field rules) is owned by the `start-training` edge function and mirrored in the browser for speed; **environment** validation (the installed ultralytics, the checkpoint, the dataset, a GPU for opt-2) can only run in Python at run start, and its failures feed environment facts back so the next run's preflight warns first. The edge function also **resolves** the effective config, since resolved values are what gets stored — and defaults come in two layers, ours resolvable server-side, ultralytics' only knowable inside the run. A run is valid from birth; drafts hold everything else.
- [Do the undeployable options come out of the dropdown now?](./issues/12-interim-remove-undeployable-options.md) — **disabled with the reason shown, not removed**: `yolo26*` needs its pipeline, pose/OBB lose a branch in compile. Minimal interim change; [06](./issues/06-fields-vs-escape-hatch.md) still owns how capability is expressed properly.
...
- **Building the YOLO26 compile pipeline.** The owner has decided YOLO26 keeps its place and gets its own path rather than being dropped ([12](./issues/12-interim-remove-undeployable-options.md)). Its 4-channel box head cannot use the on-chip `meta_arch=yolov8` NMS — the official docs confirm YOLO26 *removes* Distribution Focal Loss and defaults to an NMS-free one-to-one head — so it needs raw output plus a matching host-side decoder, and that decoder lives in `sack-detector-edge`, which this repo explicitly does not own (`AGENTS.md`). It is therefore its own effort, requiring a cross-repo contract, not a ticket here. **One lead worth checking first:** the same docs state a *one-to-many* head (traditional, with NMS) also exists, which may carry the classical DFL box and make the path far cheaper than a new decoder — unverified against a real export. What *does* stay on this map is how the parameter contract expresses more than one compile path ([06](./issues/06-fields-vs-escape-hatch.md)).
